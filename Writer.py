...
class Writer:
    ...
    def write_csv(self, aux):
        with open("res/results.tsv", mode='w') as csv_results:
    ...
    def write_csv_by_status(self, filename, aux):
        with open(filename, mode='w') as csv_results:
            ...
            for words in aux:
                result_writer.writerow([words[0]["id"], words[0]["cognate"], words[0]["language"], words[0]["cog_ID_results"],
                                        words[0]["borrowed"], words[0]["pgmc_results"], '', words[1]["id"], words[1]["cognate"],
                                        words[1]["language"], words[1]["cog_ID_results"], words[1]["borrowed"], words[1]["pgmc_results"]])
        csv_results.close()

    def write_csv_by_status_only_short(self, filename, aux):
        with open(filename, mode='w') as csv_results:
            result_writer = csv.writer(csv_results, delimiter='\t')
            result_writer.writerow(['ID_1', 'cognate_1', 'language_1','cognateID_results_1', 'cognateID_wiki_1', '',
                                    'ID_2', 'cognate_2','language_2', 'cognateID_results_2', 'cognateID_wiki_2'])
    ...
    def write_csv_only_short(self, aux):
        with open("res/results_short.tsv", mode='w') as csv_results:
            result_writer = csv.writer(csv_results, delimiter='\t')
            result_writer.writerow(['status', 'ID_1', 'cognate_1', 'language_1','cognateID_results_1', 'cognateID_wiki_1', '',
                                    'ID_2', 'cognate_2','language_2', 'cognateID_results_2', 'cognateID_wiki_2'])
            #tp
